...
const columns = [{
    ...
    formatter(value, row, index) {
        if (value) {
            return "<div class='customer-pic' style='background:url(" + (upload_server + value) + ") no-repeat center center/cover'></div>";
        } else {
            return "";
        }
    }
...
function delUser(row) {
    layer.confirm("是否确认删除管理员用户",function (){
        deleteById([row.id]);
    });
}
...
function deleteById(ids){
    layer.confirm("是否确认删除数据？", function () {
        ...
        $.ajax(url, {
            ...
            error(resp) {
                layer.msg(resp.responseJSON.error + "删除失败");
            }
        });
    });
}
...
function editById(id) {
    layer.open({
        ...
        area: ["500px", "550px"],
        maxmin: true,
        content: ctx + "/user/edit?id=" + id, //后台传递id
        btn: ["确定", "取消"],
        yes: function (index, $obj) {
            //正则表达式验证
            //
