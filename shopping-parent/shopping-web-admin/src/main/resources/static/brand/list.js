...
const columns = [{
    ...
    formatter(value, row, index) {
        if (value) {
            return "<div class='brand-logo' style='background:url(" + (upload_server + value) + ") no-repeat center center/cover'></div>";
        } else {
            return "";
        }
    }
...
function delUser(row) {
    layer.confirm("是否确认删除选中的品牌？",function () {
        deleteByIds([row.id]);
    });
}
...
function deleteByIds(ids){
        ...
        $.ajax(url, {
            ...
            traditional: true,
            success(resp) {
                layer.msg("删除成功，共删除" + resp.rows + "条记录(使用中的品牌无法被删除！)");
                $("#table").bootstrapTable("refresh");
            },
            error(resp) {
...
function editById(id){
    layer.open({
        ...
        area: ["500px", "550px"],
        maxmin: true,
        content: ctx + "/brand/edit?id=" + id, //后台传递id
        btn: ["确定", "取消"],
        yes: function (index, $obj) {
            //正则表达式验证
            //
